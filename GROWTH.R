...
# setwd('/Users/sburgess/My Drive/Data/Corals/NSF/Recharge/') # work
# setwd('/Users/scottburgess/Google Drive/Data/Corals/NSF/Recharge/') # home
...
# T9 = July/Aug 2021
# T10 = November 2021
# haplo_coral_sizes_v2.csv has data on the size of corals at each timepoint 
...
# Import data
coral_sizes <- read.csv('haplo_coral_sizes_v2.csv')
length(unique(coral_sizes$Un_ID)) # 404 corals
coral_sizesT10 <- read.csv("sampled_corals_for_haplo_v3.csv")
...
# in sampled_corals_for_haplo_v3.csv (November 2021) 
# when corals were sampled for genetic analysis.  
# Use coral_sizes for growth, and coral_sizesT10 for final size at T10.
...
# Only use corals that recruited in T3 = July/Aug 2019 or T6 = July/Aug 2020
...
Results_start_size_2020$Anova_table
EMM <- emmeans(Results_start_size_2020$model,~ Species)
pairs(EMM, simple = "Species")
...
# Function to compare allometric growth models using AICc 
...
EMM <- emmeans(Results_growth$model,~log_start_size + Species)
EMM <- emmeans(Results_growth$model,~ Species)
pairs(EMM, simple = "Species")
(exp(-0.167)-1)*100 # size 
...
add_data <- function(dat,predictions,Sp,Herbivory,Enrichment){
  ...
  if(names(dat)[9]=="Size_T10"){dat$start_size <- dat$Size_T10}
...
mtext("Fish consumer pressure treatment", side=1, line=1, cex=1.2, outer=T)
########################################################################################### 
